...
public class Casting { // 오늘의 주인공
	...
	/*
	 *  Type Casting(자료형변환 / 형변환) 중요중요중요 매우 중요함 !!! 아주아주아주 매우매우 중요함
	 *  
	 *  웹개발에서 가장 중요한 개념쓰(강사분의 주관적인 생각으로)
	 *  
	 *  자료형을 바꾸는 개념
	 *  
	 *  ☆자바에서 값을 처리하는 규칙★
	 ...
	 *  
	 *  2. 같은 자료형끼리만 연산이 가능함
	 *  => 자료형이 다른데 연산이 너무 하고 싶다?>? 둘 중 하나를 "형변환"을 해서 맞춰야함
	 *  
	 *  3. 연산의 결과물도 동일한 자료형
	 *  => 3 + 7 = 10(정수), 1.1 + 3.3 = 4.4(실수)
	 *  
	 *   [ 표현법 ] (바꾸고싶은자료형)값;
	 *   형변환연산자 / cast연산자--->(바꾸고싶은자료형) 이 부분을 칭함
	 *   
	 *   형변환의 종류 \
	 *   
...
//		System.out.println("이 내용을 콘솔창에 출력해 보세요 실습 시작!");
		...
		// 형변환은 변수 안에 있는 값에 대해서 진행하므로 변수 자체에 영향을 주는 것이 아니므로 num을 출력했을 떄 7이 나온다
		...
		//정수가 실수로 변환 될 떄 큰 사이즈의 정수타입을 작은 사이즈의 실수타입으로 자동형변환해줌
		//4Byte float 형이 long형보다 표현할 수 있는 값의 범위가 넓기 떄문이다!
		
		// 1 2 3
		// 1.000001, 1.0000002, 1.000~~~
