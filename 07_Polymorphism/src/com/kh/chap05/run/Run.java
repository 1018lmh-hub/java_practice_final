...
public class Run {
	public static void main(String[] args ) {
		...
		//요구사항이 늘어난다
		//김치찌개 이름이 네글자 이하인 김치찌개만 찾는 거 만들어주세요~~
		//9000원 이하인 김치찌개 찾는 거 만들어주세요~~
		...
		//3단계 : 매개변수 자료형 제거 : 타입추론 + 매개변수가 한 개라면 괄호도 생략 가능
		SoupFilter filter3 = (s) -> {
				return s.getMainMenu().contains("치");
			};
		//4단계 : 중괄호 안 실행문이 한 줄이라면 중괄호 + return 문 생략
		SoupFilter filter4 = s -> s.getMainMenu().contains("치");
		
		for(Soup s : soups) {
			if(filter1.test(s)) {
				System.out.println("재료에 치 들어간 찌재 : " + s.getName());
			}
		}
