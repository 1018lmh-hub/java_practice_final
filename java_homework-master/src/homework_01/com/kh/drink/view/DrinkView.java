...
public class DrinkView {
	...
	private void addDrink() {
		...
		Drink d = dc.addDrink(new DrinkDto(name, price));
		if(d != null) {
			selectAll();
	...
	private void updateDrink() {
		...
		if(dc.updateDrink(name, new DrinkDto(changeName, price))) {
	...
	private void deleteDrink() {
		...
		String name = sc.nextLine();
		if(dc.deleteDrink(name) != null) {
		System.out.println(name + " 메뉴가 삭제되었습니다.");
		selectAll();
		} else System.out.println("그런 메뉴는 존재하지 않습니다. ");
